...
const AlarmBanner = ({ alarm, onDismiss }: AlarmBannerProps) => {
  ...
  return (
    <div className="fixed top-0 left-0 right-0 z-[100] animate-slide-down safe-area-padding">
      <div
        className={`mx-3 mt-3 rounded-2xl bg-primary text-primary-foreground shadow-lg border border-primary-dark/20 flex items-center gap-3 ${
          sr ? 'p-5' : 'p-4'
        }`}
      >
        ...
        <div className="flex-1 min-w-0">
          <p className={`font-bold ${sr ? 'text-lg' : 'text-sm'}`}>
            💊 복약 시간이에요!
          </p>
          <p className={`opacity-90 ${sr ? 'text-base' : 'text-xs'} mt-0.5`}>
            {alarm.name} — {alarm.time}
          </p>
        </div>
        <button
          onClick={onDismiss}
          className={`flex-shrink-0 rounded-full bg-primary-foreground/20 hover:bg-primary-foreground/30 transition-colors ${
            sr ? 'p-3' : 'p-2'
          }`}
        >
